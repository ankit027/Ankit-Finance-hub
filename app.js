...
async function api(action,payload={}){
  ...
  let j;
  try{j=JSON.parse(text)}catch(e){throw new Error("Invalid API response. Redeploy Code.gs and confirm API URL.");}
...
// FAST SAVE: no full cloud reload after every click.
...
function item(left,right,id,table){
  return `<div class="item"><span>${left}</span><span>${right} <button class="danger" onclick="del('${table}','${id}')">Delete</button></span></div>`;
}
...
function addAsset(){
  ...
  return save("assets",{ID:uid(),"Basket ID":$("assetBasket").value,"Asset Name":$("assetName").value,"Asset Type":$("assetType").value,"Monthly Amount":num($("assetAmount").value)});
}

function renderInvest(){
  const baskets=DB.baskets||[],people=DB.people||[],assets=DB.assets||[],payments=DB.sipPayments||[];
  $("assetBasket").innerHTML='<option value="">Select basket</option>'+baskets.map(b=>{
  ...
  $("basketList").innerHTML=baskets.map(b=>{
    ...
    return `<div class="item"><span><b>${p?p.Name:""} — ${b["Basket Name"]}</b><br><small>${a.map(x=>`${x["Asset Name"]} ${money(x["Monthly Amount"])}`).join(" • ")||"No assets"}</small></span><span>${money(total)}<br>${done?"✓ PAID":`<button onclick="markBasket('${b.ID}',${total})">Mark Paid</button>`}</span></div>`;
  }).join("")||"<p>No baskets</p>";
}

function markBasket(id,total){
  return save("sipPayments",{ID:uid(),"Basket ID":id,Month:ym(),Amount:total,"Paid At":new Date().toISOString()});
}
...
function renderSplit(){
  ...
  let members=[];try{members=g?JSON.parse(g["Members JSON"]||"[]"):[]}catch(e){}
  ...
  $("splitList").innerHTML=groups.map(g=>{
    let m=[];try{m=JSON.parse(g["Members JSON"]||"[]")}catch(e){}
    const ex=(DB.splitExpenses||[]).filter(x=>x["Group ID"]===g.ID);
    return `<div class="card"><b>${g["Group Name"]}</b><p class="muted">${m.join(", ")}</p>${ex.map(e=>`<div class="item"><span>${e.Title} • ${e["Paid By"]}</span><span>${money(e.Amount)}</span></div>`).join("")||"<small>No expenses yet</small>"}</div>`;
  }).join("")||"<p>No groups</p>";
}
...
async function addSplitExpense(){
  ...
  if(!members){try{members=JSON.parse(g["Members JSON"])}catch(e){members=[]}}
