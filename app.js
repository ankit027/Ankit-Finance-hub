...
async function api(action,payload={}){
  ...
  catch(e){throw new Error("Invalid API response. Check Apps Script deployment.");}
...
async function loadAll(silent=false){
  ...
  try{
    ...
    setStatus(Object.keys(DB).length?"☁️ Offline cache":"⚠️ Sync error");
...
if($("themeBtn"))$("themeBtn").onclick=()=>{
  ...
  localStorage.setItem("financeTheme",document.body.classList.contains("dark")?"dark":"light");
  updateThemeBtn();
};
...
function item(left,right,id,table){
  return `<div class="item">
    <span>${left}</span>
    <span>${right} <button class="danger" onclick="del('${table}','${id}')">Delete</button></span>
  </div>`;
}
...
function render(){
  ...
  ].map(x=>`<div class="card"><small>${x[0]}</small><b>${money(x[1])}</b></div>`).join("");

  if($("pbList"))$("pbList").innerHTML=p.map(x=>item(`${x.Date} • <b>${x.Category}</b> • ${x.Type}`,money(x.Amount),x.ID,"passbook")).join("")||"<p>No records</p>";

  if($("salaryList"))$("salaryList").innerHTML=sal.map(x=>item(`${x.Month} • ${x.Company}`,money(x.Amount),x.ID,"salary")).join("")||"<p>No records</p>";

  const loans=DB.loans||[];
  if($("emiLoan"))$("emiLoan").innerHTML='<option value="">Select loan</option>'+loans.map(x=>`<option value="${x.ID}">${x["Loan Name"]}</option>`).join("");

  if($("loanList"))$("loanList").innerHTML=loans.map(x=>`<div class="item"><span><b>${x["Loan Name"]}</b><br><small>${x.Remarks||""}</small></span><span>${money(x["Initial Amount"])}</span></div>`).join("")||"<p>No loans</p>";

  renderSectionDashboards_();
  renderGiveTakeDashboard_();

  const tr=DB.transactions||[];
  if($("gtList"))$("gtList").innerHTML=tr.map(x=>item(`<b>${x.Person}</b> • ${x.Type}<br><small>${x.Purpose||""}</small>`,money(x.Amount),x.ID,"transactions")).join("")||"<p>No records</p>";
...
function cards_(cards){
  return `<div class="section-dashboard-grid">${
    cards.map(c=>`<div class="section-stat-card"><small>${c[0]}</small><b>${c[1]}</b>${c[2]?`<span>${c[2]}</span>`:""}</div>`).join("")
  }</div>`;
}
...
function getGiveTakeBalances_(){
  ...
  (DB.transactions||[]).forEach(x=>{
    ...
    if(!out[person]){
      out[person]={person,balance:0,given:0,received:0,taken:0,paid:0,transactions:0};
    }
    ...
    if(t.includes("give")||t.includes("lent")||t==="lend"){
      p.balance+=a;p.given+=a;
    }else if(t.includes("receive")||t.includes("received")){
      p.balance-=a;p.received+=a;
    }else if(t.includes("take")||t.includes("borrow")){
      p.balance-=a;p.taken+=a;
    }else if(t==="pay"||t.includes("paid")){
      p.balance+=a;p.paid+=a;
    }
  });
...
async function addBasket(){
  ...
  if(!person){
    person=await save("people",{ID:uid(),Name:name});
  }
...
function addAsset(){
  if(!$("assetBasket").value)return toast("Select basket");
...
function renderInvest(){
  ...
  if($("assetBasket"))$("assetBasket").innerHTML=
    '<option value="">Select basket</option>'+
    baskets.map(b=>{
      const p=people.find(x=>x.ID===b["Person ID"]);
      return `<option value="${b.ID}">${p?p.Name:""} — ${b["Basket Name"]}</option>`;
    }).join("");

  if($("basketList"))$("basketList").innerHTML=
    baskets.map(b=>{
      const p=people.find(x=>x.ID===b["Person ID"]);
      const a=assets.filter(x=>x["Basket ID"]===b.ID);
      const total=a.reduce((s,x)=>s+num(x["Monthly Amount"]),0);
      const done=payments.some(x=>x["Basket ID"]===b.ID&&x.Month===ym());

      return `<div class="item">
        <span>
          <b>${p?p.Name:""} — ${b["Basket Name"]}</b><br>
          <small>${a.map(x=>`${x["Asset Name"]} ${money(x["Monthly Amount"])}`).join(" • ")||"No assets"}</small>
        </span>
        <span>
          ${money(total)}<br>
          ${done?"✓ PAID":`<button onclick="markBasket('${b.ID}',${total})">Mark Paid</button>`}
        </span>
      </div>`;
    }).join("")||"<p>No baskets</p>";
}
...
async function addGroup(){
  ...
  const record=await save("splitGroups",{
    ID:uid(),
    "Group Name":name,
  ...
  if($("spGroupSel"))$("spGroupSel").value=record.ID;
  renderSplit();
}

function groupMembers_(g){
  try{return JSON.parse(g["Members JSON"]||"[]");}
  catch(e){return [];}
}
...
function calculateSplitGroup_(group){
  ...
  expenses.forEach(e=>{
    ...
    let participants=[];
    try{participants=JSON.parse(e["Members JSON"]||"[]");}
    catch(err){}
...
function renderSplit(){
  ...
  const select=$("spGroupSel");
  if(!select)return;

  const previous=select.value;

  select.innerHTML='<option value="">Select group</option>'+
    groups.map(g=>`<option value="${g.ID}">${g["Group Name"]}</option>`).join("");

  if(previous&&groups.some(g=>g.ID===previous))select.value=previous;
  else if(groups.length)select.value=groups[groups.length-1].ID;

  const selected=groups.find(x=>x.ID===select.value);
  const members=selected?groupMembers_(selected):[];

  if($("spPaidBy"))$("spPaidBy").innerHTML=
    '<option value="">Paid by</option>'+
    members.map(x=>`<option value="${x}">${x}</option>`).join("");

  const anchor=$("splitList");
  if(!anchor)return;
  ...
  if(!summaryBox){
    ...
    anchor.parentNode.insertBefore(summaryBox,anchor);
  }
      ...
      <div class="summary-grid">
        ...
        ${c.stats.map(s=>{
          ...
          return `<div class="card person-card">
            ...
            <span class="balance-label">${status}</span>
            <small>Paid ${money(s.paid)} • Share ${money(s.share)}</small>
          </div>`;
        }).join("")}
      </div>`;
  }else{
  ...
  anchor.innerHTML=groups.map(g=>{
    ...
    return `<div class="card">
      <b>${g["Group Name"]}</b>
      <p class="muted">${groupMembers_(g).join(", ")}</p>
      <div class="item"><span>Total spent</span><span>${money(c.total)}</span></div>
      <small>${balances||"No expenses yet"}</small>
    </div>`;
  }).join("")||"<p>No groups</p>";
}
...
async function addSplitExpense(){
  ...
  if(!members.length)members=groupMembers_(g);

  await save("splitExpenses",{
    ID:uid(),
    "Group ID":g.ID,
    Title:$("spTitle").value.trim(),
    Amount:num($("spAmount").value),
...
if($("spGroupSel"))$("spGroupSel").onchange=renderSplit;
