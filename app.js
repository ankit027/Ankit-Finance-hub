...
const API_URL = "https://script.google.com/macros/s/AKfycbwYIXL6HtbCW6QiSediymQGV_zySDfcd0f-f61zJ2ihIFJ4h1C_Ge6T_zlaVWw3-M/exec";
...
async function api(action,payload={}){
  ...
  let out;
  try{out=JSON.parse(text)}catch(e){throw new Error("Invalid server response. Check Apps Script deployment.");}
...
async function loadAll(){
  ...
  try{
    ...
    const r=await fetch(API_URL+"?action=loadAll&_="+Date.now(),{cache:"no-store"});
    ...
    console.error(e);setStatus("⚠️ Sync failed");toast(e.message||"Unable to connect");
  }
}
